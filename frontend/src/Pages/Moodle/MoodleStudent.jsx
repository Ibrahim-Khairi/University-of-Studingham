...
const MoodleStudent = () => {
  ...
  useEffect(() => {
    if (user?.levelOfStudy) setActiveYear(user.levelOfStudy);
  }, [user]);

  // Fetch published modules for the selected year
  useEffect(() => {
  ...
  // Fetch user scores when a specific module is opened
  useEffect(() => {
    if (selectedModule) {
      axios
  ...
  const submitQuiz = async () => {
    ...
    try {
      ...
      setActiveQuiz(null);
      window.location.reload(); // To lock the quiz button
    } catch (err) {
      ...
      <div className="grid grid-cols-1 lg:grid-cols-[330px_1fr] gap-4 p-5">
        ...
        <div className="overflow-hidden">
          ...
          {!selectedModule ? (
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                  {modules.map((m) => (
                    ...
                    >
                      ...
                      <p className="text-gray-400 text-[10px] font-black uppercase mt-2">
            ...
            /* --- 8-WEEK CURRICULUM VIEW (Original Design) --- */
            <div className="mt-5 bg-white p-12 rounded-[60px] shadow-2xl border-t-[20px] border-[#72333B]">
              <button
                onClick={() => setSelectedModule(null)}
                className="text-gray-400 font-black uppercase text-xs mb-8 flex items-center gap-2"
              >
                <ArrowLeft size={16} /> Back to Year {activeYear}
              </button>
                ...
                {selectedModule.weeks?.map((week) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <h3 className="text-4xl font-black text-gray-800 uppercase mb-8">
                      ...
                      <div className="bg-gray-50 p-10 rounded-[45px] text-gray-700 font-bold leading-relaxed mb-12 shadow-inner border border-gray-100">
                        {week.topic ||
                          "Week details are currently being finalized by the tutor."}
                      </div>
                          ...
                          >
                            ...
                            {slot.fileUrl ? (
                              <button
                                ...
                                className="w-full bg-[#407008] text-white py-4 rounded-2xl flex items-center justify-center gap-2 font-black uppercase text-[10px] shadow-lg"
                              >
                          ...
                          <div className="mt-16 p-10 rounded-[45px] bg-[#407008]/5 border-2 border-white shadow-sm flex justify-between items-center">
                            ...
                            {existingScore ? (
                              ...
                              <button
                                ...
                                className="bg-[#72333B] text-white px-10 py-4 rounded-3xl font-black uppercase text-sm shadow-xl hover:scale-105 transition-all"
                              >
                                Start Assessment
                              </button>
                            )}
        ...
        <div className="fixed inset-0 z-[100] bg-black/90 backdrop-blur-md flex items-center justify-center p-4">
          <div className="bg-white w-full max-w-4xl rounded-[60px] p-12 overflow-y-auto max-h-[90vh] shadow-2xl">
            <div className="flex justify-between items-center mb-12">
              <h2 className="text-4xl font-black uppercase text-[#72333B]">
                Final Exam
              </h2>
              <button onClick={() => setActiveQuiz(null)}>
                <XCircle className="text-gray-300" size={30} />
              </button>
            </div>
            ...
            <div className="flex gap-6 pt-4">
              <button
                ...
                className="flex-1 py-6 rounded-[30px] font-black text-gray-400 uppercase tracking-widest border-2 border-gray-100"
              >
                Cancel
              </button>
              <button
