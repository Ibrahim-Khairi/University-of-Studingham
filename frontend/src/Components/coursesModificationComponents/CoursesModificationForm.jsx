...
export const CoursesModificationForm = () => {
  ...
  const handleSubmit = async () => {
    try {
      ...
      const isValidTimetable = lecturePatterns.every(
        (pattern) =>
          pattern.moduleId &&
          pattern.year &&
      ...
      if (!isValidTimetable) {
        alert(
          "Timetable setup is incomplete or invalid. Please review all modules."
        );
        return;
      }

      const lectureRes = await fetch(
        `http://localhost:5000/api/lecture-patterns/course/${course._id}`,
        {
          method: "PUT",
          headers: {
            "Content-Type": "application/json",
  ...
  const handleEditSave = async () => {
    try {
      ...
      const moduleRes = await fetch(
        `http://localhost:5000/api/courses/${selectedCourse._id}/modules`,
        {
          method: "PUT",
          headers: {
            "Content-Type": "application/json",
          ...
          {[
            ...
            <div key={year.key} className="space-y-6">
              ...
              {modules[year.key].map((module, index) => (
                <div
                  key={index}
                  className="grid grid-cols-1 lg:grid-cols-2 gap-8 rounded-xl"
                >
                  <div>
                    <label className="text-[20px] block mb-2 font-bold">
                      Module Name
