      ...
      <div className="grid grid-cols-6  ">
        ...
        <div className="flex flex-col  ">
          {times.map((time) => (
            <div
              key={time}
              className="text-sm border-b-2 border-gray-400 text-gray-600 h-30"
            >
              {time}
            </div>
          ))}
          ...
          <div key={day} className="flex flex-col    ">
            {times.map((time) => {
              const lecture = lectures.find(
                (lec) => lec.day === day && lec.time === time
              );
              return lecture ? (
                <div
                  key={time}
                  className="bg-white  h-30 shadow-md p-3  border-b-2 border-gray-400 hover:shadow-lg transition"
                >
                ...
                <div
                  key={time}
                  className="h-30 border-b-2 border-gray-400"
                ></div>
              );
            })}
          </div>
